...
const ProfileCard: React.FC<ProfileCardProps> = ({
    ...
    const mouseX = useSpring(tiltX);
    const mouseY = useSpring(tiltY);

    const [isFlipped, setIsFlipped] = useState(false);

    // Tilt values (numbers)
    const rotateXValue = useTransform(mouseY, [-0.5, 0.5], [30, -30]);
    const rotateYValue = useTransform(mouseX, [-0.5, 0.5], [-30, 30]);

    // Flip value
    const flipRotation = useSpring(0, { stiffness: 120, damping: 20 });
    ...
    return (
        <motion.div
            ref={cardRef}
            className={`relative w-full h-full rounded-3xl transition-all duration-200 ${children ? 'cursor-grab active:cursor-grabbing' : 'bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md border border-white/20 shadow-xl overflow-hidden cursor-pointer'} group ${className}`}
            ...
            style={{
                ...
                rotateY: enableTilt ? rotateY : (isFlipped ? "180deg" : "0deg"), // Fallback if tilt disabled but flipped
